...
export function EmployeeForm({
  ...
  return (
    <form className="formGrid" onSubmit={handleSubmit}>
      ...
      <label>
        Currency
        <input value={values.currency} maxLength={3} onChange={(e) => updateField("currency", e.target.value.toUpperCase())} required />
      </label>
      <label>
        Date of joining
        <input type="date" value={values.dateOfJoining} onChange={(e) => updateField("dateOfJoining", e.target.value)} required />
      </label>
      <label>
        Employment type
      ...
      <label className="fullSpan">
        Manager name
        <input value={values.managerName} onChange={(e) => updateField("managerName", e.target.value)} />
      </label>
